...
template <typename T>
typename make_caster<T>::template cast_op_type<typename std::add_rvalue_reference<T>::type> cast_op(make_caster<T>&& caster)
{
...
struct type_caster < T, enable_if_t < std::is_arithmetic<T>::value&& !is_std_char_type<T>::value >>
{
    ...
    public:
        ...
        static typename std::enable_if<std::is_floating_point<U>::value, handle>::type cast(U src, return_value_policy /* policy */, handle /* parent */)
        {
...
{
    template <typename T = type, enable_if_t<std::is_same<T, handle>::value, int> = 0> pyobject_caster() : value() {}

    // `type` may not be default constructible (e.g. frozenset, anyset).  Initializing `value`
    // to a nil handle is safe since it will only be accessed if `load` succeeds.
    template <typename T = type, enable_if_t<std::is_base_of<object, T>::value, int> = 0> pyobject_caster() : value(reinterpret_steal<type>(handle())) {}
...
{
    private:
        template <typename T> arg_v(arg&& base, T&& x, const char* descr = nullptr)
    ...
    public:
        ...
        template <typename T> arg_v(const arg& base, T&& x, const char* descr = nullptr)
    ...
    private:
        ...
        bool load_impl_sequence(function_call& call, index_sequence<Is...>)
        {
            ...
            for (bool r :
                    {
                        std::get<Is>
                        (argcasters).load(call.args[Is], call.args_convert[Is])...
                    })
            {
                if (!r)
                {
