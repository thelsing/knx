...
{
    return "test_function(char *)";
}
py::str test_function4(int, float)
{
    return "test_function(int, float)";
}
py::str test_function4(float, int)
{
    return "test_function(float, int)";
}
...
namespace test_exc_sp
{
    ...
    int f4(int x) throw()
    {
        return x + 4;    // Deprecated equivalent to noexcept(true)
    }
    ...
    struct C
    {
        ...
        PYBIND11_WARNING_PUSH PYBIND11_WARNING_DISABLE_GCC("-Wdeprecated")
        PYBIND11_WARNING_DISABLE_CLANG("-Wdeprecated")
        // NOLINTNEXTLINE(modernize-use-noexcept)
        int m7(int x) throw()
        {
