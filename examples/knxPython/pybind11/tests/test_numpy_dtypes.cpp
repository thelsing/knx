...
TEST_SUBMODULE(numpy_dtypes, m)
{
    ...
    m.def("print_format_descriptors", []()
    {
        ...
        for (const auto& fmt :
                {
                    py::format_descriptor<SimpleStruct>
                    ::format(),
    ...
    m.def("buffer_to_dtype", [](py::buffer & buf)
    {
        return py::dtype(buf.request());
    });

    // test_scalar_conversion
    auto f_simple = [](SimpleStruct s)
    {
        return s.uint_ * 10;
    };
    m.def("f_simple", f_simple);
    m.def("f_packed", [](PackedStruct s)
    {
        return s.uint_ * 10;
    });
    m.def("f_nested", [](NestedStruct s)
    {
        return s.a.uint_ * 10;
    });
    ...
    auto f_simple_pass_thru = [](SimpleStruct s)
    {
        return s;
    };
    ...
    // test_str_leak
    m.def("dtype_wrapper", [](const py::object & d)
    {
        return py::dtype::from_args(d);
    });
}
