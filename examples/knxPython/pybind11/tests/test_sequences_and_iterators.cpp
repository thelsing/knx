...
TEST_SUBMODULE(sequences_and_iterators, m)
{
    ...
    m.def("make_forward_slice_size_t", []()
    {
    ...
    .def("__contains__", [](const Sequence & s, float v)
    {
        return s.contains(v);
    })
    ...
    py::class_<std::vector<NonCopyableInt>>(m, "VectorNonCopyableInt")
                                         .def(py::init<>())
                                         .def("append",
                                                 [](std::vector<NonCopyableInt>& vec, int value)
    {
...
On the actual Sequence object, the iterator would be constructed as follows:
    ...
    {
        return PySequenceIterator(s.cast<const Sequence&>(), s);
    })
#endif
    ...
    m.def("sequence_length", [](const py::sequence & seq)
    {
        return seq.size();
    });
    ...
    {
        return std::count_if(o.begin(), o.end(), [](py::handle h)
        {
            return h.is_none();
        });
    });
    ...
    {
        auto it = std::find_if(o.begin(), o.end(), [](py::handle h)
        {
            return h.is_none();
        });
        return it->is_none();
