...
TEST_SUBMODULE(stl, m)
{
    ...
    m.def("load_deque", [](const std::deque<int>& v)
    {
        return v.at(0) == 1 && v.at(1) == 2;
    });
    ...
    m.def("load_array", [](const std::array<int, 2>& a)
    {
        return a[0] == 1 && a[1] == 2;
    });
    ...
    m.def("cast_set", []()
    {
        return std::set<std::string> {"key1", "key2"};
    });
    ...
    m.def("nodefer_none_optional", [](std::optional<int>)
    {
        return true;
    });
    m.def("nodefer_none_optional", [](const py::none&)
    {
        return false;
    });
    ...
    m.def("parent_path", [](const std::filesystem::path & p)
    {
    ...
    struct visitor
    {
        ...
        result_type operator()(std::nullptr_t)
        {
...
#    endif
    ...
    m.def("stl_pass_by_pointer", [](std::vector<int>* v)
    {
        return *v;
    }, "v"_a = nullptr);
    ...
    m.def("func_with_string_or_vector_string_arg_overload", [](const std::string&)
    {
        return 3;
    });
    ...
    m.def("array_cast_sequence", [](std::array<int, 3> x)
    {
        return x;
    });
